...
public enum ErrorCode {
    ...
    ADMIN_REFRESH_TOKEN_EXPIRED(HttpStatus.UNAUTHORIZED.value(), "어드민 리프레시 토큰이 만료되었습니다. 다시 로그인을 진행해주세요."),
    ACCESS_DENIED(HttpStatus.FORBIDDEN.value(), "접근 권한이 없습니다."),
    NOT_ADMIN_USER(HttpStatus.FORBIDDEN.value(), "어드민 권한이 없는 사용자입니다."),
    REFRESH_TOKEN_NOT_EXISTS(HttpStatus.BAD_REQUEST.value(), "리프레시 토큰이 존재하지 않습니다."),
    ADMIN_REFRESH_TOKEN_NOT_EXISTS(HttpStatus.BAD_REQUEST.value(), "어드민 리프레시 토큰이 존재하지 않습니다."),
